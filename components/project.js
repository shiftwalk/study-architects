...
export default function Project({ isOpen, title, status, year, projectCode, location, images, children, i }) {
    ...
    <>
      <button onClick={() => updateOpenProjects(i)} className="border-b border-current flex items-center flex-wrap py-4 md:py-5 w-full outline-none focus:outline-none text-left">
        <span className="block list-heading w-full md:flex-1 mb-3 md:mb-0 pr-8">
          {title} <span className="list-heading--meta font-sans align-top md:-ml-1 xl:-ml-2">{children ? `(${children.length})` : ''}</span>
        </span>
                  ...
                  {children.map((e, i) => {
                    return (
                      <div className="border-b border-current">
                        <span className="list-heading--small md:flex-1 mb-3 md:mb-0 pr-8 flex items-center flex-wrap py-4 md:py-5 w-full outline-none focus:outline-none text-left border-b border-current">
                          {e.title}
                        </span>
                        
                        <m.div drag={e.images.length > 3 ? 'x' : false} dragConstraints={{ right: 0 }} dragMomentum={false} className={`w-auto pt-4 pb-3 md:pt-5 md:pb-3 whitespace-nowrap hidden md:flex ${e.images.length > 3 ? 'cursor-grab' : '' }`}>
                        ...
                        <m.div drag={e.images.length > 1 ? 'x' : false} dragConstraints={{ right: 0 }} dragMomentum={false} className={`w-auto pt-4 pb-3 md:pt-5 md:pb-3 whitespace-nowrap flex md:hidden ${e.images.length > 1 ? 'cursor-grab' : '' }`}>
                          <div className="whitespace-nowrap space-x-5 items-end w-auto">
                            {e.images.map((e, i) => {
                              let activeState = 'opacity-30'
